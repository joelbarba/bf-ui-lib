...
describe('BfObject', () => {
  ...
  describe('keyFilter', () => {
    it('should filter properties by value', () => {
      expect(BfObject.keyFilter.call(myObj, val => val === null)).toEqual({ p2: null });
      expect(BfObject.keyFilter.call(myObj, val => typeof val === 'string')).toEqual({ p1: 'hey', $p10: 'this', $q20: 'this' });
    });
    it('should filter properties by name', () => {
      expect(BfObject.keyFilter.call(myObj, 'p2, p3, p4')).toEqual({ p2: null, p3: undefined, p4: 0 });
    });
  });

  xdescribe('keyMap', () => {
    it('should change falsy values', () => {
      expect(BfObject.keyMap.call(myObj, val => (val || null))).toEqual({
        p1: 'hey',
        p2: null,
        p3: null,
        p4: null,
  ...
  describe('peel', () => {
    it('should remove null and undefined s', () => {
      expect(BfObject.peel.call(myObj)).toEqual({
        p1: 'hey',
        p4: 0,
        p5: false,
        p6: myObj.p6,
