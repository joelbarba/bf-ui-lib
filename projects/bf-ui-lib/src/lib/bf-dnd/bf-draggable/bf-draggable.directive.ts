...
export class BfDraggableDirective implements OnChanges {
  ...
  @HostListener('dragstart', ['$event']) dragstart(event) {
    ...
    ghost.style.top = '100px'; ghost.style.left = '100px';
  ...
  private setDragImage = (event, ghost) => {
    const renderedShadowRect: any = document.getElementById('bf-drag-ghost-id').getBoundingClientRect();
    const halfWidth = renderedShadowRect.width / 2;
    const halfHeight = renderedShadowRect.height / 2;
    event.dataTransfer.setDragImage(ghost, halfWidth , halfHeight);
  };
