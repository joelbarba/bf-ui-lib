...
export class BfDnDService {
  ...
  public setShadowSize = true;        // It sets with/height to "bf-drag-ghost" wrapper on dragstart, so you can use relative sizing (width:100%)
  ...
  public dragEnd = () => {
    ...
    if (this.isDropping) { // That means the drop was into a container
      this.change$.next({ eventName: 'onDragEndOk', params: {
          ...
          bfDropPlaceholder  : this.activePlaceholder
      }});
  ...
  public setDebugMode = (value) => {
    ...
    this.canvasElem = document.getElementById('bf-dnd-canvas-debugger') || document.createElement('canvas');
    if (value) {
      this.canvasElem.setAttribute('id', 'bf-dnd-canvas-debugger');
  ...
  private debugRenderCanvas(allPlaceholders, closestPlaceholder, container, event) {
    const c = document.getElementById('bf-dnd-canvas-debugger') as HTMLCanvasElement;
